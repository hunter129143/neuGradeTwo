...
void CaculaFitness()//计算个体的适应值
{
    ...
    for (int i = 0; i < Po_Size; i++)
    {
        ...
        fitness = 21.5 + x[0] * sin(4 * PI * x[0]) + x[1] * sin(20 * PI * x[1]);//适应度计算
...
void genetic_algorithm()
{
    ...
    for (int i = 0; i < nowpopulation.size(); i++)
        fileout << nowpopulation.at(i).GetVariable()[0] << "," << nowpopulation.at(i).GetVariable()[1] << "," << nowpopulation.at(i).GetFitness() << endl; //采用逗号分隔符
